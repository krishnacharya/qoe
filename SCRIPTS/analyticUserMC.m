...
%     rateMat((userVec > 0), jj) = nr_lhs / dr_lhs;
    for ii = 1 : numClasses,
        if(userVec(ii) > 0)
            vQualMat(ii, jj) = max(lminVec(ii), min(lmaxVec(ii), throughputVec(userVec(ii))));
        end
    end;
end

trm = encodeTRM_balk(arrivalRateVec, avgVideoSizeVec, gammaVec, thresVec, ...
    vQualMat, throughputVec, maxUsersVec);
...
for kk = 1:numClasses,
    ...
    trm2{kk} = encodeTaggedTRM_balk(arrivalRateVec, avgVideoSizeVec, gammaVec, thresVec, vQualMat, throughputVec, maxUsersVec, kk);
end;

% Code the badStates as +1, if the state is not possible, encode it as -1,
% goodStates are 0, absorbing states are 2 and 3
badStates = encodeBadStates_balk(vQualMat, throughputVec, maxUsersVec);
...
for kk=1:numClasses,
    ...
    probStarVec = computeProbStarvation_firstOrderMC_balk(trm2{kk},badStates(kk,:), absorbingState, vQualMat(kk,:), throughputVec, ...
        prefetchVec(kk), bVec{kk}, slope, secsPerSegVec(kk), videoRateMat(kk,:), maxUsersVec, kk, probFinishingMat(kk, :));
...
for kk=1:numClasses,
    ...
    for ii = 1:numStates,
        ...
        if (userVec(kk) < maxUsersVec(kk))
            ...
            effRateVec(ii) = throughputVec(actualUserVec);% r(i + ej)
...
for kk = 1 : numClasses,
    [avgQualVec, avgQualSwitchVec ]= computeAvgQuality_firstOrderMC_balk(trm2{kk}, badStates(kk,:), vQualMat(kk,:), throughputVec, ...
        secsPerSegVec(kk), videoRateMat(kk,:), probFinishingMat(kk,:), maxUsersVec, kk);    
    avgQualSwitches(kk) = sum(pyeTimesprobFinishingMat(kk,:) .* avgQualSwitchVec) / (sum(pyeTimesprobFinishingMat(kk,:)));
    avgQuality(kk) = sum(pyeTimesprobFinishingMat(kk,:) .* avgQualVec) / (sum(pyeTimesprobFinishingMat(kk,:)));
end
