function badStates = encodeBadStates_balk(vQualMat, rateMat, maxUsersVec)
...
for idx = 1:numStates,
    ...
    for kk=1:numClasses,
        if userVec(kk) < maxUsersVec(kk),
            ...
            if(rateMat(kk,idxPlusOne) < vQualMat(kk,idxPlusOne)) % shouldn't this also check the mimn available bit rate?
                badStates(kk,idx) = 1;
            end
        else
